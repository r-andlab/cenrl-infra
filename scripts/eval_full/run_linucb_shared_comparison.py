#!/usr/bin/env python3
"""
Compare UCB Naive, LinUCB (disjoint), and LinUCBShared (global transfer)
across China, Russia, and Kazakhstan.

10 episodes x 2000 steps. Uses canonical reverted parameters:
  FEATURES = ["categories"], UCB c=0.03, LinUCB alpha=0.5

Usage (from project root):
    python3 scripts/run_linucb_shared_comparison.py
"""
import os
import sys
...
from models.contextual_bandit.linucb_shared import LinUCBShared
...
OUTPUT_DIR = os.path.join(ROOT, "models", "outputs", f"linucb_shared_cmp_{timestamp}")
...
MODEL_STYLES = {
    ...
    "LinUCB Shared":   {"color": "#D55E00", "ls": "--",  "lw": 2.8},
}
ALL_LABELS = ["UCB Naive", "LinUCB", "LinUCB Shared"]
...
for country, gt_path in COUNTRIES.items():
    ...
    models = [
        ("UCB Naive",     UCBNaive,     {"c": 0.03, "step_size": 0.0, "initial_value_estimate": 0.0}, None),
        ("LinUCB",        LinUCB,       {"alpha": 0.5, "initial_value_estimate": 0.0}, {"action_space_klass": LinUCBActionSpace}),
        ("LinUCB Shared", LinUCBShared, {"alpha": 0.5, "initial_value_estimate": 0.0}, {"action_space_klass": LinUCBActionSpace}),
    ]

    for label, klass, extra, run_kwargs in models:
        df, cov, blk, auc = run_model(
            label, klass,
            {**base_params(cdir, label.lower().replace(" ", "_"), gt_path), **extra},
            run_kwargs,
        )
...
def plot_metric(country, metric, ylabel, cumulative, scale, fname):
    ...
    for label in ALL_LABELS:
        ...
        y  = y * scale
        x  = list(range(len(y)))
        auc = integrate(x[:MEASUREMENTS], list(y)[:MEASUREMENTS])
        s   = MODEL_STYLES[label]
        ax.plot(x, y, label=f"{label} (AUC={auc:,.0f})",
                color=s["color"], linestyle=s["ls"], linewidth=s["lw"], alpha=0.92)
    ax.set_title(country.title(), fontsize=14)
    ...
    fig.tight_layout()
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(PLOTS_DIR, f"{country}_{fname}.{ext}"),
                    dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {country}_{fname}")


for country in COUNTRIES:
    plot_metric(country, "is_blocked", "Cumulative Blocked Found", True,  1.0,   "blocked")
    plot_metric(country, "coverage",   "Coverage (%)",             False, 100.0, "coverage")

# Combined 3-panel plots
for metric, ylabel, cumulative, scale, fname in [
    ("is_blocked", "Cumulative Blocked Found", True,  1.0,   "combined_blocked"),
    ("coverage",   "Coverage (%)",             False, 100.0, "combined_coverage"),
]:
    ...
    for ax, country in zip(axes, COUNTRIES):
        for label in ALL_LABELS:
            ...
            ax.plot(x, y, label=f"{label} (AUC={auc:,.0f})",
                    color=s["color"], linestyle=s["ls"], linewidth=s["lw"], alpha=0.92)
        ax.set_title(country.title(), fontsize=13)
        ax.set_xlabel("Measurement step", fontsize=11)
        ax.set_ylabel(ylabel, fontsize=11)
        ax.tick_params(labelsize=9)
        sns.despine(ax=ax, offset=6)
    handles, labels_leg = axes[0].get_legend_handles_labels()
    fig.legend(handles, [l.split(" (")[0] for l in labels_leg],
               loc="lower center", ncol=3, fontsize=9, framealpha=0.9)
    fig.tight_layout(rect=[0, 0.08, 1, 1])
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(PLOTS_DIR, f"{fname}.{ext}"), dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {fname}")
